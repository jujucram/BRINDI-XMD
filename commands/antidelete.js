...
async function handleAntideleteCommand(sock, chatId, message, match) {
    ...
    if (!message.key.fromMe && !isOwner) {
        return sock.sendMessage(chatId, { text: '*Seul le propriétaire du bot peut utiliser cette commande.*\n> BRINDI-XMD' }, { quoted: message });
    }
    ...
    if (match === 'on') {
        ...
        return sock.sendMessage(chatId, { text: '*Commande invalide. Use .antidelete to see usage.*\n> BRINDI-XMD' }, {quoted:message});
    }

    saveAntideleteConfig(config);
    return sock.sendMessage(chatId, { text: `*Antidelete ${match === 'on' ? 'activé' : 'désactivé'}*\n> BRINDI-XMD` }, {quoted:message});
}
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        let text = `*🗑️ Deleted By:* @${deletedBy.split('@')[0]}\n` +
            `*👤 Sender:* @${senderName}\n` +
            `*📱 Number:* ${sender}\n` +
            `*🕒 Time:* ${time}\n`;
            ...
            try {
                switch (original.mediaType) {
                    case 'image':
                        ...
                        break;
                    case 'sticker':
                        // WhatsApp doesn't support caption on stickers, sending as is
                        await sock.sendMessage(ownerNumber, {
                    ...
                    case 'video':
                        ...
                        break;
                    case 'audio':
                        // WhatsApp audio messages don't support text captions directly
                        await sock.sendMessage(ownerNumber, {
