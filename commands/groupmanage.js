...
async function setGroupName(sock, chatId, senderId, text, message) {
    ...
    try {
        ...
        await sock.sendMessage(chatId, { text: `❌ Failed to update group name. ${e.message || ''}` }, { quoted: message });
    }
}
...
async function setGroupPhoto(sock, chatId, senderId, message) {
    ...
    try {
        ...
        await sock.groupUpdatePicture(chatId, { url: imgPath });
        try { fs.unlinkSync(imgPath); } catch (_) {}
        await sock.sendMessage(chatId, { text: '✅ Group profile photo updated.' }, { quoted: message });
    } catch (e) {
        await sock.sendMessage(chatId, { text: `❌ Failed to update group profile photo. ${e.message || ''}` }, { quoted: message });
    }
}
